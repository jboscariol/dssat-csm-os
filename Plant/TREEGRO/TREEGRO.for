...
!      USE Cells_2D       !From original code
      ...
      IMPLICIT NONE
      EXTERNAL TG_DEMAND, FREEZE, TG_GROW, HRES_CGRO, INCOMP, IPPLNT, 
     &  MOBIL, NFIX, NUPTAK, TG_OPGROW, TG_OPHARV, P_CGRO, PEST, 
     &  TG_PHENOL, PHOTO, PLANTNBAL, PODDET, PODS, RESPIR, TG_ROOTS, 
     &  TG_SENES, TG_VEGGR
      SAVE
!-----------------------------------------------------------------------
...
!       harvest residue.
C-GH    IF (RUN .EQ. 1 .OR. INDEX('QPF',RNMODE) .LE. 0) THEN
        IF (RUN .EQ. 1) THEN				
        HARVRES % RESWT  = 0.0
        HARVRES % RESLIG = 0.0
        HARVRES % RESE   = 0.0
...
C-----------------------------------------------------------------------
     ...
     &  TOSHMINE,TOCHMINE,HPODWT,HSDWT,HSHELWT)           !Output
      ...
      ELSE
        !PCNVEG = -99.    !Wait for GBuild fix for -99s
        PCNVEG = 0.
      ENDIF
